...
import { AlertCircle, CheckCircle, Clock, Filter, Fullscreen, MapPin, Package, Search, Star, Users } from 'lucide-react';
...
const ProductDetail = () => {
    ...
    // For managing inventory per variant
    const [variantStock, setVariantStock] = useState({});
    const [currentVariantKey, setCurrentVariantKey] = useState('');

    // Generate variant key for inventory tracking
    ...
    // Get variant price (if different variants have different prices)
    ...
    useEffect(() => {
        ...
        const fetchProduct = async () => {
            try {
                ...
                // Initialize variant stock (in real app, this would come from API)
                const variants = {};
                if (productData.product.colors && productData.product.sizes) {
                    productData.product.colors.forEach(color => {
    ...
    useEffect(() => {
        ...
        // Reset quantity when variant changes to ensure it doesn't exceed new variant's stock
    ...
    useEffect(() => {
        const checkWishlistStatus = async () => {
            ...
            try {
                ...
                toast(error.message || 'Error checking wishlist status: Please try again.');
            }
        };
    ...
    const handleQuantityChange = (change) => {
        ...
        if (newQuantity >= 1 && newQuantity <= maxStock) {
            ...
            const variantInfo = selectedColor || selectedSize ?
                ` for ${selectedColor ? selectedColor : ''}${selectedColor && selectedSize ? ' in size ' : ''}${selectedSize ? selectedSize : ''}` : '';
            showToast.error(`Sorry, only ${maxStock} items available in stock${variantInfo}.`);
        }
    };

    // Enhanced color selection handler
    const handleColorSelect = (color) => {
        setSelectedColor(color);

        // If this color + current size combo has no stock, try to find an available size
        if (selectedSize) {
            ...
            if (stockForVariant === 0 || stockForVariant === undefined) {
                // Try to find a size that has stock with this color
                ...
                if (availableSize) {
                    ...
                    showToast.info(`Switched to size ${availableSize} as selected combination was out of stock.`);
                } else {
                    showToast.warning('This color is currently out of stock in all sizes.');
                }
            }
        }
    };

    // Enhanced size selection handler
    const handleSizeSelect = (size) => {
        setSelectedSize(size);

        // If this size + current color combo has no stock, try to find an available color
        if (selectedColor) {
            const newVariantKey = generateVariantKey(selectedColor, size);
            const stockForVariant = variantStock[newVariantKey];

            if (stockForVariant === 0 || stockForVariant === undefined) {
                // Try to find a color that has stock with this size
                ...
                if (availableColor) {
                    ...
                    showToast.info(`Switched to ${availableColor} as selected combination was out of stock.`);
                } else {
                    showToast.warning('This size is currently out of stock in all colors.');
                }
            }
        }
    };

    // Get color display name and hex value
    const getColorInfo = (colorName) => {
        ...
        return colorMap[colorKey] || { name: colorName, hex: colorName.toLowerCase() };
    };
    ...
    const handleAddToCart = async () => {
        try {
            ...
            if (currentStock <= 0) {
                showToast.error('This variant is currently out of stock.');
                return;
            }

            if (quantity > currentStock) {
                showToast.error(`Only ${currentStock} items available for this variant.`);
                return;
            }
            ...
            const confirmView = window.confirm('Would you like to view your cart?');
            if (confirmView) {
                router.push('/cart');
            }
        } catch (err) {
            console.error('Error adding to cart:', err);
            showToast.error(err.message || 'Failed to add product to cart. Please try again.');
        } finally {
                        ...
                        <div className="p-8 lg:p-12 space-y-8">
                            ...
                            <div className="space-y-4">
                                ...
                                >
                                    ...
                                    <span className={`px-4 py-2 rounded-full text-sm font-medium backdrop-blur-sm border ${currentStock > 0
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-lg font-semibold text-gray-200">
                                            Color: <span className="font-normal text-gray-400 capitalize">
                                                {selectedColor ? getColorInfo(selectedColor).name : 'Select a color'}
                                            </span>
                                        </h3>
                                            ...
                                            return (
                                                ...
                                                >
                                                    {isSelected && (
                                                        ...
                                                        >
                                                            <FaCheck
                                                                className={`text-xl ${colorInfo.hex === '#ffffff' || colorInfo.hex === '#f5f5dc' || colorInfo.hex === '#ffd700'
                                                                    ? 'text-gray-800'
                                                                    : 'text-white'
                                                                    } drop-shadow-lg`}
                                                            />
                            ...
                            {product.sizes && product.sizes.length > 0 && (
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <h3 className="text-lg font-semibold text-gray-200">
                                            Size: <span className="font-normal text-gray-400">
                                    ...
                                    <button
                                        ...
                                        onClick={() => showToast.info('Size guide feature coming soon!')}
                                    >
                                        📏 Size Guide
                                    </button>
                                </motion.div>
                            ...
                            {/* Stock Alert for Current Selection */}
                                ...
                                >
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-orange-200 font-medium">
                                            Only {currentStock} left in stock for this variant!
                                        </span>
                                    </div>
                                </motion.div>
                            )}
                                ...
                                <div className="flex items-center gap-6">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="flex items-center border-2 border-gray-600/50 rounded-2xl overflow-hidden bg-gradient-to-r from-gray-800/50 to-gray-900/50 backdrop-blur-sm shadow-lg">
                                            ...
                                            <span className="px-6 py-3 font-bold text-xl text-gray-200 min-w-[60px] text-center">
                                                {quantity}
                                            </span>
                                            <button
                                                onClick={() => handleQuantityChange(1)}
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div className="p-4 bg-gradient-to-r from-gray-700/30 to-gray-800/50 backdrop-blur-sm rounded-2xl border border-gray-600/20">
                                        ...
                                        <span className="font-semibold text-gray-200">
                                            {isValidSelection()
                                                ? `${currentStock} units (selected variant)`
                            ...
                            <div>
                                ...
                                <p className="text-gray-400 mt-1">
                                    {product.numReviews > 0
                                        ? `Showing ${Math.min(3, recentReviews.length)} of ${product.numReviews} reviews`
                                        : 'No reviews yet'
                                    }
                                </p>
                            </div>
                                ...
                                <div className="flex items-center gap-4">
                                    <Link
                                        ...
                                        onClick={() => {
                                            ...
                                            router.push(`/products/${product._id}/reviews?data=${encoded}`);
                                        }}
                        ...
                        {relatedProducts.map((relatedProduct) => (
                            ...
                            >
                                <div className="bg-gradient-to-br from-gray-800/50 via-gray-900/80 to-black/90 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-700/30 overflow-hidden transition-all duration-500 group-hover:shadow-2xl group-hover:shadow-amber-400/10 group-hover:-translate-y-2 group-hover:border-amber-400/30">
                                    ...
                                    <div className="p-4">
                                        <h3 className="text-lg font-semibold text-gray-200 mb-2 truncate group-hover:text-white transition-colors">
                                            {relatedProduct.name}
                                        </h3>
                                        <div className="flex items-center justify-between">
